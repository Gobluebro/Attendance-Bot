...
function checkArguments(args) {
  ...
  if (args[0] == undefined || args[1] == undefined) {
    ...
    errorLog =
      'You have too many arguments. There should only be 2 \n' +
      'e.g. !attendadd 2017/12/25 username';
    return errorLog;
    ...
    errorLog =
      'The command you entered is not formated correctly \n' +
      'e.g. !attendadd 2017/12/25 username';
    return errorLog;
  }
  return errorLog;
...
client.on('message', async message => {
  ...
  if (message.member.roles.some(r => [config.moderatorName].includes(r.name))) {
    switch (command) {
      ...
      //#endregion
      //attendview
      //#region attendcurrent
      case 'attendcurrent':
        if (config.isRecording) {
          var formattedString = '';
          for (var i = 0; i < config.attendanceArray.length; i++) {
        ...
        fs.unlink('./logs/' + thisMonth + '.txt', function(err) {
          if (err) {
            if (err.code === 'ENOENT') {
              return message.reply(
                'File not found\n' +
                  'Make sure format is in year/month\n' +
                  'e.g. !attenddeletemonth 2017/12'
              );
            }
          }
          return message.reply(
            `The attendance for ${args[0]} has been removed.`
          );
        });
        break;
      //#endregion
      ...
      case 'attendadd':
        ...
        if (config.isRecording) {
          ...
          fs.open('./logs/' + thisMonth + '.txt', 'r', (err, fd) => {
            ...
            fs.readFile('./logs/' + thisMonth + '.txt', 'utf8', function(
              err,
              data
            ) {
              if (err) throw err;
...
Make sure you have the correct format e.g, !attendremove 2017/12/25 username`);
          ...
          fs.readFile('./logs/' + thisMonth + '.txt', 'utf8', function(
            ...
            if (nextDayIndex < lastPartFile.indexOf(args[1])) {
              ...
              var combine =
                firstPartFile + removeUser.replace(/(\r\n|\n|\r)/, '');
              console.log(combine);
              fs.writeFile(
                './logs/' + thisMonth + '.txt',
                combine,
                ...
                err => {
                  if (err) throw err;
                  return message.reply(
                    `${args[1]} has been removed from the recording on ${
                      args[0]
                    }`
                  );
                }
      ...
      case 'attendstop':
        if (config.isAutomated) {
          ...
          if (config.isRecording) {
            ...
            return message.reply(
              `Attendance Bot is not currently recording. If you wish to start the recording please use the command "!attendstart" to start.`
            );
          }
        }
        break;
